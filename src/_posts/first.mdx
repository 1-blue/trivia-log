---
title: 'react-query'
description: 'react-query에 대한 기본 개념과 사용 예시'
tags: [FrontEnd, ReactQuery]
icon: ''
image: ''
...
> 해당 포스트는 [`blegram`](https://github.com/1-blue/blegram)이라는 솔로 프로젝트를 만들면서 사용했던 `react-query`에 대한 내용을 기록하는 포스트입니다.<br />
...
```diff-tsx {1,3-4} showLineNumbers
npm i react-query
+ ㅁㄴㅇㅁㄴㄴㅁㅇ
- ㅁㄴㅇㅁㄴㄴㅁㅇ
* ㅁㄴㅇㅁㄴㄴㅁㅇ
...
+ [`MyReactQueryProvider.tsx`](https://github.com/1-blue/blegram/tree/master/src/providers/MyReactQueryProvider.tsx)
...
> 자세한 인자와 반환 값에 대해서는 [react-query - `useQuery<T>(key, fetcher[, options])`](https://tanstack.com/query/v4/docs/react/reference/useQuery)를 참고해주세요!<br />

+ 매개변수
  1. `key`: 유일한 식별자로 사용 ( 배열도 가능하며, 배열로 작성하지 않아도 배열로 등록됨 )
  1. `fetcher`: 데이터를 패칭하는 함수 ( 반드시 `Promise` 반환 / 예외 처리 )
  1. `options`: 너무 많은 옵셥들이 있어서 설명은 생략하고 필요한 부분은 [공식 문서](https://tanstack.com/query/v4/docs/react/reference/useQuery)를 참고해주세요!<br />
...
+ [`useMe.ts`](https://github.com/1-blue/blegram/tree/master/src/hooks/query/useMe.ts)
...
> 자세한 인자와 반환 값에 대해서는 [react-query - `useInfiniteQuery<T>(key, queryFn[, options])`](https://tanstack.com/query/v4/docs/react/reference/useInfiniteQuery)를 참고해주세요!<br />

+ 매개변수
  1. `key`: 유일한 식별자로 사용 ( 배열도 가능하며, 배열로 작성하지 않아도 배열로 등록됨 )
  1. `queryFn`: 다음 페이지의 데이터를 패칭하는 함수이고, 첫 번째 인자에 객체로 `pageParam`로 현재 페이지 값이 들어옴 ( 반드시 `Promise` 반환 / 예외 처리 )
  1. `options`: `getNextPageParam()`에서 반환한 값이 `queryFn`의 `pageParam`로 들어감 ( 다른 옵션들은 [공식 문서](https://tanstack.com/query/v4/docs/react/reference/useInfiniteQuery)를 참고해주세요! )
...
+ 반환 값
  ...
  1. ... 다른 반환 값들은 [공식 문서](https://tanstack.com/query/v4/docs/react/reference/useInfiniteQuery)를 참고해주세요!
...
+ [`usePosts.ts`](https://github.com/1-blue/blegram/tree/master/src/hooks/query/usePosts.ts)
...
> 자세한 인자와 반환 값에 대해서는 [react-query - `useMutation(mutationFn[, options])`](https://tanstack.com/query/v4/docs/react/reference/useMutation)와 [react-query - `queryClient.setQueryData(queryKey, updater)`](https://tanstack.com/query/v4/docs/react/reference/QueryClient#queryclientsetquerydata)를 참고해주세요!<br />

+ 매개변수
  1. `mutationFn`: 네트워크 요청을 수행할 함수 ( 반드시 `Promise` 반환 / 예외 처리 )
  1. `options`: `onSuccess()`를 이용해서 변경사항을 반영 ( 다른 옵션들은 [공식 문서](https://tanstack.com/query/v4/docs/react/reference/useMutation)를 참고해주세요! )
...
+ [`useDeletePost.ts`](https://github.com/1-blue/blegram/tree/master/src/hooks/query/useDeletePost.ts)
...
# 📮 레퍼런스
1. [Udemy - react-query](https://kmooc.udemy.com/course/react-query-react)
1. [react-query - `useQuery<T>(key, fetcher[, options])`](https://tanstack.com/query/v4/docs/react/reference/useQuery)
1. [react-query - `useInfiniteQuery<T>(key, queryFn[, options])`](https://tanstack.com/query/v4/docs/react/reference/useInfiniteQuery)
1. [react-query - `useMutation(mutationFn[, options])`](https://tanstack.com/query/v4/docs/react/reference/useMutation)
2. [react-query - `queryClient.setQueryData(queryKey, updater)`](https://tanstack.com/query/v4/docs/react/reference/QueryClient#queryclientsetquerydata)